...
def train_total(data_dir, data_list='train.txt'):
    ...
    path_total=read_labeled_image_list(data_dir+'/train', data_list)
    images = []
    masks = []
    ...
    for i in range(len(path_total)):
        ...
        image = cv2.resize(img, (320, 320), interpolation=cv2.INTER_LINEAR)
        images.append(image)    

        lab_path = path_total[i][1]
        mas = cv2.imread(lab_path)
        mask = cv2.resize(mas, (320,320), interpolation=cv2.INTER_LINEAR)
    ...
    return [images_total, masks_total],[masks_total,masks_total*images_batch_new]
