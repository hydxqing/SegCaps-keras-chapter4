...
from utils.load_2D_data import train_total,generate_val_batches
...
def train(args, u_model, net_input_shape):
...
# POC testing, change stride from 20 to args.stride in generate_val_batches
   ...
   # gen_data = datagen.flow_from_directory(path, batch_size=1, shuffle=True,target_size=(512, 512))
   #
   #  history = model.fit_generator(
   #      generate_train_batches(args.data_root_dir,args.batch_size),
   #      max_queue_size=8, workers=4, use_multiprocessing=args.use_multiprocessing,
   #      steps_per_epoch=args.steps_per_epoch,
   #      validation_data=generate_val_batches(args.data_root_dir,args.batch_size),
   ...
   #      verbose=1)
    ...
    x,y=train_total(args.data_root_dir)
